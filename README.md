...
- **It can't make up numbers.** Every other tool's AI assistant admits it hallucinates. Ours can't — it calls exact, deterministic reports (not guessed SQL), so the answer is the real computed number or nothing. And this isn't a promise, it's CI: [the agreement test](internal/api/agreement_test.go) asserts the MCP answer and the HTTP API answer are identical for the same question — every build, forever. There is no second query path that can drift.
